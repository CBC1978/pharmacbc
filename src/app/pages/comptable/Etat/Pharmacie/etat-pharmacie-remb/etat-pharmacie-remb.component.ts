...
@Component({
  selector: 'app-etat-pharmacie-remb',
  standalone: true,
  imports: [SidebarAdminComponent,CommonModule,FormsModule],
  templateUrl: './etat-pharmacie-remb.component.html',
  styleUrl: './etat-pharmacie-remb.component.css'
})
export class EtatPharmacieRembComponent {
  ...
  ngOnInit(){
    ...
    this.etatService.Get_etat_id_pharmacie(this.etat_id).subscribe(res=>{
  ...
  Afficher_etat_remb() {
    this.etatService.Read_etat_remboursement_pharmacie(this.etat_id).subscribe(response => {
  ...
  pdf_etat(): void {
    this.etatService.Generate_pdf_etat_pharmacie(this.etat_id).subscribe((data: Blob) => {
